...
auto WatchedDirectory::Allocate(std::string_view watchDirectory, FileChangedCallback cb) -> std::optional<WatchedDirectory> {
...
void FileChangeWatcher::PollEvents(WatchedDirectory const& directory) noexcept {
    ...
    while (i < bytesWritten) {
        ...
        if (event->mask & IN_MODIFY) {
            directory.OnChangeCallback()({event->name, event->len}, isDirectory);
        }
    }
  }
}