...
struct CpuView {
    ...
    explicit CpuView()
        : data(nullptr), dataEnd(nullptr), byteStride(0) {}
    explicit CpuView(T* data, size_t numElements, size_t byteStride = sizeof(T))
        : data(reinterpret_cast<uint8_t const*>(data)), dataEnd(reinterpret_cast<uint8_t const*>(data) + numElements*byteStride), byteStride(byteStride) {
